...
const Users = ({users, isAuth, getUsers, delUsers, addUser, editUser}) => {
    ...
    const onChangeInputSearch = (e) => {
        setFiltredItems(users.filter(el => el.fullname.toLowerCase().indexOf(e.target.value.toLowerCase()) >= 0));
    ...
    return (
        <div className='users-wrapper'>
            ...
            {itemsFiltred.map((user) => <User key={user.id} user={user} delUsers={delUsers} editUser={editUser}/>)}
